...
const client = postgres(connectionString, {
  ...
  connection: {
    application_name: 'ai-resumeforge',
  },
  onnotice: (notice) => console.log('Postgres notice:', notice),
  onparameter: (key, value) => console.log('Parameter change:', key, value),
});
...
async function ensureTablesExist() {
  ...
  const taskStatusResult = await client`
    SELECT EXISTS (
      SELECT FROM information_schema.tables 
      WHERE table_schema = 'public' 
      AND table_name = 'TaskStatus'
    );`;

  if (!taskStatusResult[0].exists) {
    await client`
