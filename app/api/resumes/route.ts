...
export async function GET(request: Request) {
  try {
    ...
    ipRoyaltiesMap.forEach((royalties, ipId) => {
      ...
      const rewardAmount = royalties.reduce(
        (sum, royalty) => sum + Number(royalty.amount) * 0.001,
        0
      );
    ...
    const decoratedResumes = resumes.map((resume) => {
      // 이력서와 관련된 IP ID 찾기 - cid 또는 id로 직접 연결된 IP만 찾기
      const relatedIPs = userIPs.filter(
        (ip) => String(ip.cid) === String(resume.id) || String(ip.cid) === resume.cid
      );
